	...
	if(window.io) {
		...
		socket.on('connect', function(){
			...
			socket.on('riot:message', function(message){
				console.log(message);
				var transposeValue = (parseInt(message.args[0].value)+180)/360*12
				$("#pitch-control-transpose").val(transposeValue).change();
				$("#x-control").val(parseInt(message.args[1].value)).change();
				$("#y-control").val(parseInt(message.args[2].value)).change();
			});
		});
	}
